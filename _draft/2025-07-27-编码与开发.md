...
关键是你要在读书期间做出能代表这个L几L几的项目，不然出去了别人怎么信你。
...
以TPU、Catapult、Crest 和 Pixel Visual Core为例，用深度神经网络作为典型应用域，介绍了 MLP、CNN、RNN/LSTM 的基本计算特征，分析了参数规模、权重数量、每层操作数和 operations/weight，并指出 DNN 工作负载的本质是若干高频核心算子：矩阵-向量乘、矩阵-矩阵乘、卷积/模板（stencil）、ReLU、Sigmoid、Tanh，再说明 batch 和 quantization 如何提升运算强度和能效。
...
Google TPU（Tensor Processing Unit）是专门为深度神经网络设计的 ASIC 加速器。TPU 的核心是一个 256×256 的 8-bit 矩阵乘单元（systolic array），能够高效执行大规模矩阵运算，这是 DNN 中最主要的计算。它还配备了软件管理的大容量片上缓冲（unified buffer），用于存储中间激活数据，以及专门的权重缓存结构。TPU 作为 CPU 的协处理器（通过 PCIe 连接）工作，专注执行计算密集型任务，从而大幅提升能效。TPU 的指令集主要包括从主机内存读取数据到片上缓冲（Read_Host_Memory）、读取权重到权重 FIFO（Read_Weights）、执行矩阵乘、卷积等核心运算（MatrixMultiply / Convolve）、执行激活函数（Activate）、将结果写回主存（Write_Host_Memory）等。其中最关键的是矩阵乘指令：它以流水线方式处理输入（B×256）与固定权重（256×256），逐周期输出结果。这说明 TPU 的 ISA 是围绕 “数据流 + 矩阵运算”设计的，而不是通用计算。TPU 将大量资源用于计算单元和本地存储，而控制逻辑和接口占比很小。这体现了 DSA 的设计原则——尽量减少数据搬运、增加计算密度。在TPU的数据流中，矩阵乘采用 systolic array（脉动阵列）结构：数据在阵列中像波一样流动，每个处理单元只做局部计算并传递部分结果。
...
DDR（Double Data Rate SDRAM）和 SDR（Single Data Rate SDRAM）是接口形式，描述存储芯片和控制器之间的数据传输方式，前者在时钟的上升沿河下降沿都采样数据，后者只在其中一种沿采样数据。DDR 是 DRAM 的一种，数据传输速率单位为 MT/s（Mega Transfers per second，百万次传输/秒），描述的是 DDR 芯片引脚（I/O 引脚）与内存控制器（SoC内部的硬件模块）之间，每秒完成了多少次数据传输。。
...
RTL 仿真和 EMU 仿真的波形抓取和查看、PMU 工具的使用和分析：在 SIPU Programming Guide，以及收藏夹的几篇文档中有介绍。单个 sikernel case 生成 arch_model vector dump 有专门的文档。
