...
其中 IO 时间按一次读入 $\mathbf{A}$、一次读入 $\mathbf{B}$、一次写回 $\mathbf{C}$ 估算：
...
![Arch Model架构图](https://cdn.jsdelivr.net/gh/HatrixXXX/Hatrix-s-Blog-Image/img/whiteboard_exported_image_nowatermark_no_sipu.png)

目前ESL Model的架构图

![ESL Model架构图-system](https://cdn.jsdelivr.net/gh/HatrixXXX/Hatrix-s-Blog-Image/img/whiteboard_exported_image_nowatermark_no_sipu111.png)

![ESL Model架构图-PE](https://cdn.jsdelivr.net/gh/HatrixXXX/Hatrix-s-Blog-Image/img/whiteboard_exported_image_nowatermark_no_sipu222.png)

性能概况：SIPU 具有 32 个 Core，频率 1250MHz。

对于计算部分

- TMAC（Tensor MAC）性能：以 TF32 Tensor Core计量，MAC per PE 为 512，因此TF32的理论算力为$32 \times 512 \times 1250 \text{M} \times 2 = 40.96\text{TFLOPS}$。

- TALU性能：以FP32 Vector计量，MAC per PE 为 64，因此FP32的理论算力为$32 \times 64 \times 1250 \text{M} \times 2 = 5.12\text{TFLOPS}$。

对于访存部分

- L2 Buffer per PE为512B/cycle/PE，DRAM size为32GB，
