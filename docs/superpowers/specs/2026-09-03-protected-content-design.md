...
所有通过校验的文章都会生成页面。`locked` 可以省略，省略时按 `false` 处理：

| `locked` | 构建结果 |
| --- | --- |
| `false` | 发布公开页面，正文是普通静态 HTML |
| `true` | 发布公开 Hero 和解锁外壳，正文与正文图片只输出密文 |

加锁文章仍公开标题、发布日期、类型、阅读时间、稳定 URL 和封面。文章卡片、归档和相邻文章显示锁标记，加锁文章不进入公开搜索索引。封面永远公开，可以继续使用符合图床固定提交规则的远程 URL。
