...
> 历史说明：文章分类与标签设计已由 `2026-09-03-remove-taxonomy-design.md` 废止，原始导航由 `2026-09-03-navigation-content-hubs-design.md` 取代，Banner 和波浪边界以 `2026-09-03-non-home-banner-design.md` 为准，页脚以 `2026-09-03-compact-legal-footer-design.md` 为准。其余差异保留为当时的实施记录。
...
Expected: PASS in all three projects; no duplicate mobile content or oversized blank region. Current suite: 63 Playwright cases and 21 Windows-generated snapshots. Snapshot names are platform-neutral; the Pages workflow intentionally does not run the visual suite.
