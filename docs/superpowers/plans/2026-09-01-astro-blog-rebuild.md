...
- 迁移文章前先完成 `docs/superpowers/specs/2026-09-01-image-delivery-optimization-design.md` 对应的图片管线；Astro 直接消费生成后的完整图片和缩略图 URL，不重复压缩远程图片。
...
export function toPostFrontmatter(fileName: string, source: Record<string, any>, updatedDate?: string) {
  ...
  return {
    ...
    ...(source.image?.thumbnail ? { thumbnail: String(source.image.thumbnail) } : {}),
...
Both homepage routes call `getCollection('posts', ({ data }) => !data.draft)` then `sortPosts` and `paginatePosts(..., 6)`. Page 1 lives at `/`; pages 2+ are generated by `src/pages/page/[page].astro`. Each card renders `thumbnail ?? cover`, title, excerpt, date, reading time, category and `postPath(legacySlug)`. The article hero and social metadata continue to use `cover`.
...
Authorize `cdn.jsdelivr.net` through Astro image remote patterns. Add the remark plugin after `remark-math`。Do not transform remote images during the Astro build; covers, thumbnails and body images already come from the independent optimization pipeline.
...
Expected: the report count equals the unique URLs extracted from migrated content and has no unexplained difference from the optimization manifest; clean build completes under 8 minutes. Inline images retain `loading="lazy"` and `decoding="async"`.
