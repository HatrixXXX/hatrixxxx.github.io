...
- [ ] **Step 2: 删除所有文章和草稿中的 taxonomy frontmatter**
...
- [ ] **Step 3: 删除 taxonomy helper，收窄搜索文档**
...
`src/content/posts/2025-07-04-杂货铺.md` 保持未暂存，直到最终交付时同时保留用户正文改动和本次 frontmatter 删除。
...
- [ ] **Step 2: 删除导航、卡片和文章页 taxonomy UI**
...
Expected: legacySlug 总数为 40；Giscus 配置与测试仍在；《杂货铺》的 diff 同时包含用户新增链接和本次 frontmatter 删除，正文链接原样保留。
...
Expected: 开发服务器监听 `http://127.0.0.1:4321/` 并保持运行。最终汇报通过的命令、真实计数、提交列表，以及未暂存的《杂货铺》文件中哪些 hunk 属于用户、哪些属于本次删除。
