...
加强三段同侧测试：分别记录三段轨迹刚生成时的 alpha 总和。第三段产生后等待 350ms，前两段的 `alphaSumInRect` 必须仍不低于各自基线的 50%，第三段保持大于 0；随后轮询三块为 0。旧实现只保留 raster 像素，前两段会在约 200ms 内降到肉眼不可见，这一步必须 RED。
