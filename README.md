...
首页是固定在视口内的全屏封面，显示站名、循环打字文案和博客入口。页面空闲时会预取 `/blog/`；点击底部箭头，或用鼠标、触摸向左拖动封面，都可通过向左揭开动画进入博客总览。首页没有滚动条，也不渲染页脚、Sakana 或 CursorTrail。其他页面的顶部图片统一为桌面和平板 `240px`、手机 `200px`，所有题图直接衔接正文，不再使用波浪分隔；文章摘要和发布时间等信息位于题图下方。
...
| `pnpm check:site` | 旧文章路由、CNAME、4038 条站内链接和发布体积 |
| `pnpm test:e2e` | Chromium 的桌面、平板和手机检查，共 177 项；其中 18 张视觉快照在 Windows 生成，文件名不含平台后缀 |
