...
主导航依次为首页、博客文章、作品橱窗、关于我和留言板。博客文章与关于我包含二级菜单；`/blog/` 是全部文章索引，`/blog/<type>/` 按文章类型筛选。关于子页目前是占位页，`/guestbook/` 使用 Giscus 保存留言。
...
| `pnpm test:e2e` | Chromium 的桌面、平板和手机检查，共 91 项；其中 18 张视觉快照在 Windows 生成，文件名不含平台后缀 |
