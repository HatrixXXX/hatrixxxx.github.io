...
私有内容仓库有 41 篇已发布文章，公开仓库不跟踪任何已发布文章的 Markdown。所有通过校验的文章都会生成页面；一次完整构建会生成 71 个 HTML 页面。
...
页面返回按钮按固定路径导航：六个文章列表页返回 `/blog/`，文章详情返回所属类型页，其余页面统一返回首页。

非首页主导航在顶部按视口居中，依次为首页、博客文章、作品橱窗、书签、软件、装备、计划、实验场、友链、留言板。桌面端博客文章保留横向二级菜单；宽度小于 `1200px` 时改用同顺序的折叠菜单。搜索与主题切换在桌面右侧，管理员退出按钮在左侧，不影响导航居中。关于我仍由首页卡片进入。`/blog/` 是博客入口，`/blog/all/` 显示全部文章，`/blog/<type>/` 按文章类型筛选。关于子页目前是占位页，`/guestbook/` 使用 Giscus 保存留言。

站内搜索支持普通文章的标题和正文。结果会显示命中片段并高亮关键词；点击后文章页会滚动到对应段落，段落背景短暂突出后渐变恢复。加锁文章不会进入公开搜索索引。

首页采用参考图的两侧透视卡片排布，中央展示透明角色立绘，位于背景之上、入口面板之下。`1920×1080` 画布按有效 CSS 视口等比缩放；超宽屏将左右两组移向屏幕边缘，组内排布保持不变，竖屏保留上下空白。`src/pages/index.astro` 用 `leftPanels` 定义左侧四角，用 `rightPanels` 定义右侧平面内矩形；右侧全部卡片共用父层的一次 `matrix3d` 投影，主要行列间隔为 12 个设计像素。首页没有滚动条，也不渲染普通导航栏、页脚、Sakana 或 CursorTrail。

左上以小图标提供站内搜索，中左显示圆形等级与生日年度进度；左下是金句、常驻播放器、友链和留言板。金句语料位于 `src/data/home-quotes.ts`，每 15 秒切换，双击或键盘 Enter/空格可立即切换，也可暂停；系统要求减少动态效果时默认暂停自动切换。右上辉光管时钟每秒显示本地时间，格式为 `YYYY/MM/DD HH:mm:ss`，卡片只比博客入口略长。博客卡片直接进入 `/blog/`，再由该页进入“全部文章”和五种文章类型，其余卡片跳转到作品、关于、书签、软件、装备、计划及实验场。收藏区的书签占左侧整高，右侧上方是不可点击的“工具推荐”标题，下方并排软件和装备；加长后的计划与实验场共同占满底部一行，已移除辅助空槽。书签、软件、装备和友链复用 `/about/` 下的独立页面；`/plans/` 与 `/lab/` 暂为空状态。旧首页的打字、底部箭头和拖动揭开逻辑已移除。布局细节见[首页设计记录](docs/superpowers/specs/2026-09-25-home-dashboard-design.md)。

现有普通页面 Hero 统一为桌面和平板 `240px`、手机 `200px`。文章详情以标题与元信息开头，不渲染题图横幅；全站不使用波浪分隔。

当前文章页启用页脚，显示版权、“保留所有权利”和站点统计；普通布局默认不显示页脚。版权区不显示第三方许可链接，许可文件仍发布在 `/third-party-notices.txt`。

`SidebarStack` 承载作者资料，站点统计由页面的 `showStats` 决定；当前关于页及其子页只显示作者资料。文章侧栏显示目录和最近文章。音乐播放器独立于侧栏，在首页卡片中常驻，在所有非首页页面显示左侧唱片入口。访客数由生产环境的 `https://events.vercount.one/js` 提供，开发和测试环境不请求该服务；加载失败时只保留 `—`，不影响页面内容。
...
`id`、`title`、`artist` 和 `src` 必填，`cover` 可省略。首页播放器嵌在左下透视卡片内，始终展开。所有非首页页面默认折叠到左侧，只露出半张唱片；点击唱片展开，点击卡片空白、标题或收起按钮可折叠，播放、进度和音量控件不会触发折叠。播放器通过 `transition:persist` 保留 DOM 和 Audio，客户端页面导航时继续播放，并按当前路径切换首页或侧边模式。播放列表为空时不会创建 `Audio` 对象，也不会请求音频文件。
...
| `pnpm build` | 图片预检、71 页静态构建与加锁内容泄漏审计 |
| `pnpm check:protected` | 现有 `dist/` 的加锁正文、资源、索引和 sitemap 泄漏审计 |
| `pnpm check:site` | 旧文章路由、CNAME、3977 条站内链接和发布体积 |
...
完整浏览器套件尚有旧断言和基线待同步，包括分页文章卡片数量、页脚及非首页视觉基线；213 是测试收集数量，不代表全部通过。本次未改这些页面的既有行为。
...
326 项单元测试、18 项视觉回归、Astro 检查、构建、图片预检、保护审计和 3977 条站内引用检查通过。相关浏览器检查覆盖导航、搜索、播放器、主题切换、深浅主题与减少动态效果；未运行完整端到端套件。
