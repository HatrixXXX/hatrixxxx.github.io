...
首页采用参考图的两侧透视卡片排布，中央角色区域留空。`1920×1080` 画布按有效 CSS 视口等比缩放并完整居中显示，窗口尺寸和浏览器缩放变化时保持卡片的相对位置；竖屏保留上下空白，不改成纵向卡片列表。卡片四角记录在 `src/pages/index.astro`，由 `HomePanel.astro` 调用 `projectPanel` 生成 `matrix3d` 投影。首页没有滚动条，也不渲染普通导航栏、页脚、Sakana 或 CursorTrail。

左上搜索复用站内搜索，中左显示 Hatrix；左下是金句、常驻播放器、友链和留言板。金句语料位于 `src/data/home-quotes.ts`，每 15 秒切换，双击或键盘 Enter/空格可立即切换，也可暂停；系统要求减少动态效果时默认暂停自动切换。右上辉光管时钟每秒显示本地时间，格式为 `YYYY/MM/DD HH:mm:ss`。博客卡片直接进入 `/blog/`，再由该页进入“全部文章”和五种文章类型，其余卡片跳转到作品、关于、书签、软件、装备、计划及实验场。“收藏推荐”仅作分组标题。书签、软件、装备和友链复用 `/about/` 下的独立页面；`/plans/` 与 `/lab/` 暂为空状态。旧首页的打字、底部箭头和拖动揭开逻辑已移除。布局细节见[首页设计记录](docs/superpowers/specs/2026-09-25-home-dashboard-design.md)。

其他页面的顶部图片统一为桌面和平板 `240px`、手机 `200px`，所有题图直接衔接正文，不再使用波浪分隔；文章摘要和发布时间等信息位于题图下方。
...
| `pnpm check:site` | 旧文章路由、CNAME、3976 条站内链接和发布体积 |
| `pnpm test:e2e` | Chromium 的桌面、平板和手机检查，当前收集 211 项；保留 18 张 Windows 视觉基线，文件名不含平台后缀 |
...
完整浏览器套件尚有旧断言和基线待同步，包括分页文章卡片数量、页脚及非首页视觉基线；211 是测试收集数量，不代表全部通过。本次未改这些页面的既有行为。
