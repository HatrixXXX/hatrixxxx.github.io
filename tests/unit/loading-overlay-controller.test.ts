...
import { createLoadingController, isHomepageDestination } from '../../src/scripts/loading-overlay';
...
describe('loading overlay controller', () => {
  ...
  it('holds homepage loading for the full reveal and collapse sequence', () => {
    vi.useFakeTimers();
    const show = vi.fn();
    const finishAnimation = vi.fn();
    const hide = vi.fn();
  ...
  it('recognizes homepage destinations from URL-like event values', () => {
    expect(isHomepageDestination(new URL('https://hatrix.site/'))).toBe(true);
    expect(isHomepageDestination({ pathname: '/' })).toBe(true);
    expect(isHomepageDestination('/')).toBe(true);
    expect(isHomepageDestination(new URL('https://hatrix.site/blog/'))).toBe(false);
    expect(isHomepageDestination({ pathname: '/blog/' })).toBe(false);
  });
});
