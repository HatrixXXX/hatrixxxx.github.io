...
时钟管理模块 CMT（Clock Management Tiles）是专用于产生/处理/分发时钟的硬件资源，提供了时钟合成、倾斜矫正和过滤抖动等功能，用于将外部时钟转化为 FPGA 可用的高质量时钟，CMT 中包含 PLL（Phase Lock Loop） 、MMCM（Mixed Mode Clock Manager）和 Clock Buffers。
