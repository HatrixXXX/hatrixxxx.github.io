...
87. 关于复位，数据路径不需要复位逻辑，因为其正确性会由伴随的控制信号保障，加上复位没有必要反而会增加硬件负担。其它信号推荐设计成局部复位。

88. 一般情况下信号变量不要直接使用乘法 *、除法 /、求余数 % 等操作。这些操作符被综合后，结构和时序往往不易控制。应该使用相关优化后的 ip 模块或工艺库中的集成模块。但是 parameter 类型的常量就可以使用此类操作符，因为在编译之初编译器就会计算出常量运算的结果，不会消耗多余的硬件资源。
