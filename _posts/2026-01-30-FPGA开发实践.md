...
70. 一般情况下信号变量不要直接使用乘法 *、除法 /、求余数 % 等操作。这些操作符被综合后，结构和时序往往不易控制。应该使用相关优化后的 ip 模块或工艺库中的集成模块。但是 parameter 类型的常量就可以使用此类操作符，因为在编译之初编译器就会计算出常量运算的结果，不会消耗多余的硬件资源。乘法可以直接写，vivado可以直接综合成DSP Slice，有对应的硬件资源。 

71. 将wcfg添加到工程中，可以指定默认加载的波形配置。

72. block design中make external的端口要右键在external port properties中选择clock port，不然export platform导出xsa文件时会报错。

73. 新建工程的时候不要勾选vitis扩展平台，否则在vitis中建立的application的flow navigator中会没有run和debug的选项。

74. 暂时先不要把自己写的RTL模块直接放到Block Design里面，不方便debug，而且本来就能用的不要重新增加设计成本。除了能让自己看着舒服一点之外没有任何实质性的价值。
