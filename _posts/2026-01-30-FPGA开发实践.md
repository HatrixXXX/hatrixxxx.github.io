...
不同的 HP 口可以配置不同的数据和地址总线宽度以及其它一系列参数。(不同主设备的数据交互接口可能不一样)。多个 HP 可以用于流量隔离(把不同功能流量分到不同口，避免相互干扰)、QoS 策略、简化仲裁/设计(每个 PL 主控对接固定 HP)，或在 PL 侧做 AXI interconnect 后把多个从机合并上 PS。
...
37. compute_rgb 在改的时候遇到两个问题，一是 fifo 的深度需要设置 32 才能不漏后面的数据，通过看 data count 发现的；另一个是 xyz 和 sh 没有对上导致只有第一次的三个结果是对的，后面的 rgb 的前一组 sh 用的是上一组的 y。后来又发现了一次深度不够导致只有第一个 burst 的三个高斯的数据正确以及第二个 burst 的第一个高斯数据正确，后面全乱了，fifo 深度改到 64 就对了。再后来又由于改了些其他模块的代码导致这个模块的 sh en 的时机不对，现象是只有第一个高斯的 rgb 正确，后面都不对，通过对比 sh terms 发现是后面的 rgb 的算第一拍 sh terms 的时候用了上一个高斯末尾一拍的 sh 系数，即 sh 的 rd en 少保持了一拍导致少切换了一个。
...
75. 中间结果位宽不够，导致得到的 cov2d 完全不对，渲染比较大的高斯会出问题。

76. 向 DDR 写入的时候多半都需要 fifo 缓存，数据本身需要同步，数据和写入的地址之前也需要同步。之前用 burst 只给出首地址不需要考虑这个问题，像素写入时地址不是规律递增的，需要手动维护地址，就需要考虑数据和地址同步的问题了。

75. write kv 只有第一拍收到 WREADY 的原因，是和后面的 write pix 打架了。给 write pix 模块加上 step ready，将 step_notice0_done 传入，让 write kv 结束了再 write pix 而不是只根据 fifo valid 发起 req。
